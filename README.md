...
当前主线版本是“无头自动续登版”。它包含桌面管理器、账号管理面板、OpenAI 兼容 API、模型映射、限流、工具调用格式兼容、工具别名统一、日志查看、崩溃自动重启，以及 token 过期后的无头自动登录刷新。

如果你之前使用的是“有头登录版”，区别是：有头版刷新账号时会弹出浏览器登录窗口；无头版会在后台启动浏览器完成登录，并在桌面面板里显示正在登录的账号和排队状态。
...
- 打包后的桌面管理器无命令行黑框
- 默认使用无头登录刷新 token，不弹出浏览器窗口
- 程序启动后自动检测账号状态，过期或即将过期的账号会逐个登录刷新
- token 剩余时间低于 5 分钟时会提前刷新，减少对话中断
- 自动登录队列会在桌面面板显示“正在无头登录”和“等待登录”状态
...
- 高频状态轮询日志已静音，日志里主要保留模型请求、工具调用、错误和账号刷新记录

## 版本区分

### 无头自动续登版（当前主线）

- 适合长期运行、局域网共享、给 Hermes/Codex 等客户端稳定调用。
- 账号 token 过期后会自动在后台刷新。
- 登录过程不会弹出浏览器窗口，适合无人值守运行。
- UI 会显示当前正在登录的账号，避免用户误以为程序无响应。

### 有头登录版（旧版行为）

- 账号刷新时会显示浏览器窗口。
- 适合需要人工观察登录过程、排查验证码/风控页面的场景。
- 如果账号经常遇到微软额外验证，可以临时切回有头模式排查。
...
2. 解压 `M365CopilotGateway-v1.1.0-headless-windows.zip`。
...
发布包名带有 `headless` 的版本是无头自动续登版，不会覆盖旧的有头版说明和旧发布包。
